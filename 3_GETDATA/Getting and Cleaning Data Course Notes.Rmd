...
* `gather()` 
    ...
    * *example*: `gather(students, sex, count, -grade)` --> gather each key (in this case named sex), value (in this case count) pair into one row
		* effectively translates to (columnName, value) withe new names imposed on both --> all combinations of column name and value
...
* `today()`, `now()` --> returns the current date and time, with extractable parts (hour(), month())
    * `tzone = "America/New_York"` --> can use the "tzone" argument to specify time zones (list [here](http://en.wikipedia.org/wiki/List_of_tz_database_time_zones))
